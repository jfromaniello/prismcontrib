...
{
    /// <summary>
    /// Defines a class that manages publication and subscription to events that can be dispatched to classes with thread affinity.
    /// This includes Windows Forms and WPF classes, or other classes that have thread affinity based on the SynchronizationContext class
    /// pattern of .NET 2.0 and later. If the subscriber subscribes from the thread(s) for which they have thread affinity, they can can specify
    /// through the ThreadOption that they want to be notified on the thread for which they have affinity. This will work for Windows Forms or WPF, 
    /// dispatching the call to the UI thread. 
    /// </summary>
    /// <typeparam name="TPayload">The event argument payload that will be sent to each subscriber.</typeparam>
    public class CompositeEvent<TPayload> : EventBase
    {
        ...
        /// <see cref="CompositeSynchronizationContextEvent{TPayload}"/> will maintain a <seealso cref="WeakReference"/> to the target of the supplied <paramref name="action"/> delegate.
        /// </summary>
        ...
        /// <summary>
        /// Publishes the <see cref="CompositeSynchronizationContextEvent{TPayload}"/>.
        /// </summary>
